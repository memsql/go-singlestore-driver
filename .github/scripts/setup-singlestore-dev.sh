...
TEST_DATABASE=${S2_TEST_DB_NAME:-test}
...
if [[ "${EXISTS}" -eq 0 ]]; then
    docker run -d \
        --name ${CONTAINER_NAME} \
        -v ${PWD}/.github/test/ssl:/test-ssl \
        -v ${PWD}/.github/test/jwt:/test-jwt \
...
mysql -u root -h 127.0.0.1 -P $S2_MASTER_PORT -p"${SINGLESTORE_PASSWORD}" -e 'create user "root-ssl"@"%" require ssl'
mysql -u root -h 127.0.0.1 -P $S2_AGG_PORT_1  -p"${SINGLESTORE_PASSWORD}" -e 'create user "root-ssl"@"%" require ssl'
...
mysql -h 127.0.0.1 -u root -P $S2_MASTER_PORT -p"${SINGLESTORE_PASSWORD}" -e "SET GLOBAL data_conversion_compatibility_level = '6.0'"
mysql -h 127.0.0.1 -u root -P $S2_MASTER_PORT -p"${SINGLESTORE_PASSWORD}" -e "CREATE USER 'test_jwt_user' IDENTIFIED WITH authentication_jwt"
